...
import { _0xPayContractAddress, bondableChains, metadata } from './config'
...
class HopBridge extends Base {
  ...
  public async pay (
    ...
    console.log('in pay %s %s %s', tokenAmount, orderId, ammWrapperAddress)
    const populatedTx = await this.populatePayTx(tokenAmount, orderId, ammWrapperAddress)
    return this.signer.sendTransaction(populatedTx)
  }

  public async populatePayTx (
    tokenAmount: TAmount,
    orderId: string,
    tokenContractAddress: string
  ):Promise<any> {
    const singerAddress = await this.getSignerAddress()
    ...
    const l2pay = await this.getL2Pay(Chain.Polygon)
    const populatedTx = await l2pay.populateTransaction.charge(...txOptions)
    return populatedTx
  }
  ...
  public async estimatePayGasLimit (
    tokenAmount: TAmount,
    orderId: string,
    sourceChain: TChain,
    options: Partial<SendOptions> = {}
    ...
    console.log(ammWrapperAddress)
    console.log('tokenAmount, %s, %s', tokenAmount, orderId)
    const populatedTx = await this.populatePayTx(tokenAmount, orderId, ammWrapperAddress)

    return this.getEstimatedPayGasLimit(Chain.Polygon, populatedTx)
  }

  private async getEstimatedPayGasLimit (
    sourceChain: TChain,
    populatedTx: any
  ) {
    sourceChain = this.toChainModel(sourceChain)
  ...
  public async getSendEstimatedGasCost (
    ...
    console.log('getSendEstimatedGasCost ' + estimatedGasLimit + ', ' + gasPrice)
  ...
  public async getSendData (
    ...
    const bonderFeeRelativePromise = BigNumber.from(1000)
  ...
  public async getL2Pay (chain: TChain, signer: TProvider = this.signer) {
    chain = this.toChainModel(chain)
    // const bridgeAddress = this.getL2BridgeAddress(this.tokenSymbol, chain)
    const bridgeAddress = '0xe4e79Cc3D40b3A95202e00a14154De6342e4CA70'
    if (!bridgeAddress) {
      throw new Error(
        `token "${this.tokenSymbol}" on chain "${chain.slug}" is unsupported`
      )
    ...
    return L2Pay__factory.connect(_0xPayContractAddress, provider)
  }
